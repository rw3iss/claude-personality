...
2. Then scan the result for a `SESSION:` trailer line and act on it **immediately, in this session**:
   ...
   - No `SESSION:` line — informational command (`list`, `status`, `create`); just present the output. After a successful `create`: Read the new profile file, replace its `> ...` description line (via Edit) with a punchy one-line summary of the persona in the style of the built-in profiles (voice + a sample phrase, e.g. `> Salty high-seas pirate captain — arr, matey.`), then show the user the final profile text.
